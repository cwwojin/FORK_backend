...
router
    .get(       // GET : get stampbooks by query - userId, facilityId
        ...
        [
            ...
            body('facilityId', `body field 'facilityId' must be a positive integer`).exists().isInt({min: 1}),
            body('sellerId', `body field 'sellerId' must be a postivie integer`).exists().isInt({min: 1}),
            body('type', `body field 'type' must be one of ${TRANSACTION_TYPES}`).exists().isIn(TRANSACTION_TYPES),
            body('amount', `body field 'amount' must be non-negative integer`).exists().isInt({min: 0}),
            validatorChecker,
        ],
