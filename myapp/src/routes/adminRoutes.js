...
router
  ...
  .get(
    ...
    [
      ...
      query(
        "type",
        `optional query field 'type' must be one of ${REPORT_TYPES}`
      )
        .optional()
        .isIn(REPORT_TYPES),
      query(
        "status",
        `optional query field 'status' must be one of ${REPORT_STATUS}`
      )
        .optional()
        .isIn(REPORT_STATUS),
      validatorChecker,
  ...
  .post(
    ...
    [
      ...
      body("type", `body field 'type' must be one of ${REPORT_TYPES}`)
        .exists()
        .isIn(REPORT_TYPES),
      body("content", `body field 'content' must be string`)
        .exists()
        .isString(),
      body(
        "reviewId",
        `optional body field 'reviewId' must be a positive integer`
      )
        .optional()
        .isInt({ min: 1 }),
      validatorChecker,
  ...
  .post(
    ...
    [
      ...
      body("adminId", `body field 'adminId' must be a positive integer`)
        .exists()
        .isInt({ min: 1 }),
      body("action", `body field 'action' must be a string`)
        .exists()
        .isString(),
      validatorChecker,
    ],
